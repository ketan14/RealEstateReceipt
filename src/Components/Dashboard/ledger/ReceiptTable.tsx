...
export const ReceiptTable = ({ groups }: { groups: GroupedReceipt[] }) => {
    ...
    const handlePrint = async (receipt: any) => {
        try {
            setLoadingMap((prev) => ({ ...prev, [receipt.receipt_id]: true }));
            await printReceipt(receipt);
        } catch (err) {
            console.error("Print failed:", err);
        } finally {
            setLoadingMap((prev) => ({ ...prev, [receipt.receipt_id]: false }));
        }
    };
    return (
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl">
            <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-800/60 text-sm">
                    {groups.map((group) => (
                        <tr key={group.id} className="hover:bg-slate-850/30 transition-colors align-top" style={{ backgroundColor: group.total_amount_paid === group.agreed_sale_value ? 'green' : 'default' }}>
                                    ...
                                    {group.all_receipts
                                        ...
                                        .map((receipt) => (
                                            <button
                                                ...
                                                disabled={loadingMap[receipt.receipt_id] === true}
                                                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium transition-all duration-200 group w-full justify-center whitespace-nowrap
                          ${loadingMap[receipt.receipt_id]
                                                        ? "bg-slate-700 text-slate-400 cursor-not-allowed"
                                                        : "bg-indigo-600/20 hover:bg-indigo-600/40 border border-indigo-500/40 hover:border-indigo-400 text-indigo-300 hover:text-indigo-100"
                                                    }`}
                                            >
                                                {loadingMap[receipt.receipt_id] ? (
                                                    ...
                                                    <>
                                                        ...
                                                        >
                                                            <path
                                                                ...
                                                                d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.75 19.5H17.25l-.72-5.671M6.72 13.829A9.006 9.006 0 0112 12.75c2.388 0 4.558.94 6.15 2.475"
                                                            />
                                                        </svg>
                                                        Print{" "}
                                                        {receipt.receipt_number.split("-")[1] ||
                                                            receipt.receipt_number}
                                                    </>
                                                )}
                                            </button>
